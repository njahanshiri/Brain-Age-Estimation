...
class age_model_3DCNN:
    def __init__(self, data_type, log_path):

        self.data_provider = MRI_DataProvider.BrainMRI_DataProvider()
        self.data_type = data_type
        self.log(log_path, data_type)



    def log(self, log_path, data_type):
        self.CSV_logger = CSVLogger(os.path.join(log_path, "log.csv"), append=True,
                                    separator=';')
        self.TensorBoard = TensorBoard(log_dir=log_path, batch_size=opt.batch_size_3D, update_freq='epoch')
        checkpoint_path = os.path.join(log_path,
                                       data_type + "_3DCNN.{epoch:02d}-{loss:.2f}.hdf5")
    ...
    def train(self, data_generator_Train, data_generator_valid, epochs= opt.epochs_3D, optimizer= opt.optimizer, loss= opt.loss, model_path=None):
    ...
    def create_data_test(self):
        ...
        return  self.data_provider.get_test_data(self.data_type, None)
